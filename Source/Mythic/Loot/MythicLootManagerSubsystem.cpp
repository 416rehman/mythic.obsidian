...
#include "../Inventory/MythicWorldItem.h"
#include "Kismet/GameplayStatics.h"
#include "Mythic/Inventory/MythicInventoryComponent.h"

UMythicItemInstance *UMythicLootManagerSubsystem::Create(TSubclassOf<UItemDefinition> item_def, int32 quantity_if_stackable, AActor *owner) {
...
AMythicWorldItem *UMythicLootManagerSubsystem::CreateAndSpawn(TSubclassOf<UItemDefinition> item_def, int32 quantity_if_stackable, const FVector &location, float radius,
...
AMythicWorldItem *UMythicLootManagerSubsystem::CreateAndGive(TSubclassOf<UItemDefinition> item_def, int32 quantity_if_stackable, UMythicInventoryComponent *inventory, AActor *owner,
    ...
    UMythicItemInstance *ItemInstance = Create(item_def, quantity_if_stackable, owner);

    // Give the item to the player
    return inventory->AddItem(ItemInstance, is_private);
}

void UMythicLootManagerSubsystem::EmulateDropPhysics(const FVector &location, float radius, AActor *owner, bool is_private, UE::Math::TVector<double> start_location,
...
AMythicWorldItem *UMythicLootManagerSubsystem::Spawn(UMythicItemInstance *item, const FVector &location, float radius, AActor *owner, bool is_private = false) {
    // Make sure the default world item class is set and the item instance is valid
    if (!DefaultWorldItemClass) {
        UE_LOG(Mythic, Warning, TEXT("Default world item class is not set - Use SetDefaultWorldItemClass to set it."));
        return nullptr;
    ...
    for (auto &Hit : OverlappingWorldItems) {
        AMythicWorldItem *OverlappingWorldItem = Cast<AMythicWorldItem>(Hit.GetActor());
    ...
    AMythicWorldItem *WorldItem = GetWorld()->SpawnActor<AMythicWorldItem>(DefaultWorldItemClass, start_location, FRotator::ZeroRotator);
...
void UMythicLootManagerSubsystem::SetDefaultWorldItemClass(TSubclassOf<AMythicWorldItem> NewDefaultWorldItemClass) {
