...
public:
    ...
    AMythicWorldItem *AddItem(UMythicItemInstance *ItemInstance, bool is_private);
    ...
    UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category = "Slots")
    bool DropItem(UMythicInventorySlot *item_slot, const FVector &location, const float radius = 100.0f);

    // Picks up a WorldItem. Returns true if the item was picked up and the WorldItem was destroyed.
    // UFUNCTION(BlueprintCallable, BlueprintAuthorityOnly, Category = "Slots")
    // int32 PickupItem(AMythicWorldItem *world_item);
