...
#include "Mythic/Loot/MythicLootManagerSubsystem.h"
...
void UMythicInventoryComponent::PopulateListView(UListView *SlotListView) {
    if (SlotListView) {
...
AMythicWorldItem * UMythicInventoryComponent::AddItem(UMythicItemInstance *ItemInstance, bool is_private) {
    ...
    if (amount_added != original_qty) {
        ...
        UMythicLootManagerSubsystem *loot_manager = GetOwner()->GetGameInstance()->GetSubsystem<UMythicLootManagerSubsystem>();
        AMythicWorldItem *world_item = loot_manager->Spawn(ItemInstance, GetOwner()->GetActorLocation(), 100, GetOwner(), is_private);
        if (world_item) {
            return world_item;
        }
    }
...
bool UMythicInventoryComponent::DropItem(UMythicInventorySlot *item_slot, const FVector &location, const float radius) {
    AActor *lOwner = GetOwner();
    checkf(lOwner != nullptr, TEXT("GetItem:: Invalid Inventory Owner"));
    checkf(lOwner->HasAuthority(), TEXT("AddToAnySlot:: Called without Authority!"));
    ...
    AMythicWorldItem *world_item = loot_manager->Spawn(item_slot->GetItemInstance(), location, radius, GetOwner(), true);
...
// int32 UMythicInventoryComponent::PickupItem(AMythicWorldItem *world_item) {
    // AActor *lOwner = GetOwner();
