...
#include "MythicTimeOfDayController.h"

#include "Mythic.h"
#include "Components/LightComponent.h"

// Sets default values
AMythicTimeOfDayController::AMythicTimeOfDayController():
    NightLightIntensity(0),
    DaytimeDirectionalLight(nullptr),
    NighttimeDirectionalLight(nullptr) {
...
void AMythicTimeOfDayController::OnConstruction(const FTransform& Transform) {
...
void AMythicTimeOfDayController::BeginPlay() {
	Super::BeginPlay();

	// Start a timer to update the time of day based on the morning, afternoon, evening, and night lengths
	GetWorldTimerManager().SetTimer(this->TimeOfDayTimerHandle, this, &AMythicTimeOfDayController::UpdateTimeOfDay,
...
float AMythicTimeOfDayController::GetSunPositionForCurrentTime() {
...
void AMythicTimeOfDayController::SetTime(int32 hour, int32 minute, int32 second) {
...
int AMythicTimeOfDayController::GetDayOfMonth() {
...
int AMythicTimeOfDayController::GetMonth() {
...
int AMythicTimeOfDayController::GetYear() {
	return this->CurrentTimeOfDay.GetDays() / 360 + 1;
}

void AMythicTimeOfDayController::UpdateSunAndMoon() {
...
void AMythicTimeOfDayController::MulticastSyncTimeOfDay_Implementation(const FTimespan& Timespan) {
	this->CurrentTimeOfDay = Timespan;
	UE_LOG(Mythic, Warning, TEXT("Time of day synced to %s"), *this->CurrentTimeOfDay.ToString());
}

void AMythicTimeOfDayController::UpdateTimeOfDay() {
...
EDayTime AMythicTimeOfDayController::GetDayTime(int32 current_hour) {
