...
void UMythicNavigationComponent::OnMoveForwardTriggered(const FInputActionValue& Value) {
	if (CachedPlayerCharacter) {
		CachedPlayerCharacter->AddMovementInput(UGameplayStatics::GetPlayerCameraManager(this, 0)->GetActorForwardVector() * 100000, Value.Get<float>(), false);
	}
}

void UMythicNavigationComponent::OnMoveRightTriggered(const FInputActionValue& Value) {
	if (CachedPlayerCharacter) {
		CachedPlayerCharacter->AddMovementInput(UGameplayStatics::GetPlayerCameraManager(this, 0)->GetActorRightVector() * 100000, Value.Get<float>(), false);
	}
}
