// Mythic — status-buildup decay unit tests.
// Covers the pure decay math the server-side buildup falloff (UMythicLifeComponent::ApplyRegen ->
// UMythicAttributeSet_Defense::DecayAllBuildups) is built on. The live per-attribute decay is server-driven and
// PIE-verified; this locks the math.
// Run via: Session Frontend → Automation → Mythic.Combat.StatusBuildupDecay
...
bool FMythicStatusBuildupDecayTest::RunTest(const FString &Parameters) {
    // ComputeBuildupAfterDecay(Cur, DecayPerSecond, DeltaSeconds) = max(0, Cur − decay×dt)
    ...
    // Sustained-vs-sporadic intuition: a single ~30 hit decays away over a few ticks; sustained hits outpace decay.
    float B = 30.0f;
    for (int32 i = 0; i < 10; ++i) {
        B = UMythicAttributeSet_Defense::ComputeBuildupAfterDecay(B, 10.0f, 0.5f); // 5/tick
    }
