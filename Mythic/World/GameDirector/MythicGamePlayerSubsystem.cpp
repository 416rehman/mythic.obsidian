...
void UMythicGamePlayerSubsystem::OnLevelBeginMakingVisible(UWorld *World, const ULevelStreaming *LevelStreaming, ULevel *Level) {
    ...
    UE_LOG(Mythic, Warning, TEXT("Level %s is being made visible. Level Streaming State: %d"), *LevelStreaming->GetFNameForStatID().ToString(), State);

    UE_LOG(Mythic, Warning, TEXT("List of all the actors in the level:"));
    for (AActor *Actor : Level->Actors) {
        if (!Actor) {
            continue;
        }
        UE_LOG(Mythic, Warning, TEXT("Actor: %s"), *Actor->GetName());
    }
    ...
    if (auto local_player = this->GetLocalPlayer()) {
        ...
        if (player_controller == nullptr) {
            UE_LOG(Mythic, Error, TEXT("Player controller is null"));
            return;
        }

        auto pawn = player_controller->GetPawn();
        if (pawn == nullptr) {
            UE_LOG(Mythic, Error, TEXT("Pawn is null"));
            return;
        }
        auto player_location = pawn->GetActorLocation();
        UE_LOG(Mythic, Warning, TEXT("Player location: %s"), *player_location.ToString());

        // Check if the player is in the level
        auto LevelTransform = LevelStreaming->LevelTransform;
        UE_LOG(Mythic, Warning, TEXT("Level transform: %s"), *LevelTransform.ToString());
        UE_LOG(Mythic, Warning, TEXT("Level location: %s"), *LevelTransform.GetLocation().ToString());
        UE_LOG(Mythic, Warning, TEXT("Level rotation: %s"), *LevelTransform.GetRotation().ToString());
        UE_LOG(Mythic, Warning, TEXT("Level scale: %s"), *LevelTransform.GetScale3D().ToString());
...
void UMythicGamePlayerSubsystem::OnLevelBeginMakingInvisible(UWorld *World, const ULevelStreaming *LevelStreaming, ULevel *Level) {
    UE_LOG(Mythic, Warning, TEXT("Level %s is being made invisible. Level Streaming State: %d"), *Level->GetName(), LevelStreaming->GetLevelStreamingState());
    ...
    for (AActor *Actor : Level->Actors) {
        ...
        UE_LOG(Mythic, Warning, TEXT("Actor: %s"), Actor == nullptr ? TEXT("Actor is null") : *Actor->GetName());
    }
}
...
void UMythicGamePlayerSubsystem::OnLevelStreamingStateChanged(UWorld *World, const ULevelStreaming *LevelStreaming, ULevel *Level,
    ...
    if (Level) {
        UE_LOG(Mythic, Warning, TEXT("Level %s has changed state. Level Streaming State: %d"), *Level->GetName(), static_cast<uint8>(LevelStreamingState));
    }
    else {
        UE_LOG(Mythic, Warning, TEXT("Level may not be loaded. Level Streaming State: %d"), static_cast<uint8>(LevelStreamingState));
    }
}
