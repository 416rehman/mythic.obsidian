...
struct FAffixesRuntimeReplicatedData {
    ...
    TArray<FRolledAttributeSpec> RolledCoreAffixes;

    UPROPERTY(BlueprintReadWrite)
    TArray<FRolledAttributeSpec> RolledAffixes;
...
/**
 * AffixesFragment can be used to modify attributes without any additional logic. It can be used to add core stats to an item, or to add random affixes to an item.
 */
...
public:
    ...
    // Applies affixes
    static void ApplyAffixes(UAbilitySystemComponent *ASC, TArray<FRolledAttributeSpec> &InRolledAffixes);
    // Removes applied affixes
    static void RemoveAffixes(UAbilitySystemComponent *ASC, TArray<FRolledAttributeSpec> &InRolledAffixes);

    // Checks if an affix is already rolled
    static bool IsAffixRolled(const FGameplayAttribute &Affix, TArray<FRolledAttributeSpec> &InRolledAffixes);
